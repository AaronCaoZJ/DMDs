...
class SDDecoupledGuidance(SDGuidance):
    ...
    def compute_decoupled_loss(
        ...
        with torch.no_grad():
            ...
            # DM: 鼓励 generator 输出接近 teacher 的条件预测
            dm_update_vector = (pred_dm_real_cond_image - pred_dm_fake_cond_image)
            dm_norm_factor = (pred_dm_real_cond_image - pred_generator_image).abs().mean(dim=[1,2,3], keepdim=True)
        ...
        loss_dict = {
            "loss_decoupled": loss,
            "loss_ca": loss_ca,
            "loss_dm": loss_dm  # 和原来的 dm_loss 区分
        }
